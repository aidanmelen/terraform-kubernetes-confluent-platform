...
func TestTerraformConfluentPlatformExample(t *testing.T) {
	terraformOptions := &terraform.Options{
		...
		TerraformDir: "../examples/confluent_platform",
	...
	expectedConnect := "map[apiVersion:platform.confluent.io/v1beta1 kind:Connect metadata:map[name:connect namespace:confluent] spec:map[dependencies:map[kafka:map[bootstrapEndpoint:kafka:9071]] image:map[application:confluentinc/cp-server-connect:7.2.0 init:confluentinc/confluent-init-container:2.4.0] replicas:1]]"
	expectedKsqlDB := "map[apiVersion:platform.confluent.io/v1beta1 kind:KsqlDB metadata:map[name:ksqldb namespace:confluent] spec:map[dataVolumeCapacity:10Gi image:map[application:confluentinc/cp-ksqldb-server:7.2.0 init:confluentinc/confluent-init-container:2.4.0] replicas:1]]"
	expectedControlCenter := "map[apiVersion:platform.confluent.io/v1beta1 kind:ControlCenter metadata:map[name:controlcenter namespace:confluent] spec:map[dataVolumeCapacity:10Gi dependencies:map[connect:[map[name:connect url:http://connect.confluent.svc.cluster.local:8083]] ksqldb:[map[name:ksqldb url:http://ksqldb.confluent.svc.cluster.local:8088]] schemaRegistry:map[url:http://schemaregistry.confluent.svc.cluster.local:8081]] image:map[application:confluentinc/cp-enterprise-control-center:7.2.0 init:confluentinc/confluent-init-container:2.4.0] replicas:1]]"
	expectedSchemaRegistry := "map[apiVersion:platform.confluent.io/v1beta1 kind:SchemaRegistry metadata:map[name:schemaregistry namespace:confluent] spec:map[image:map[application:confluentinc/cp-schema-registry:7.2.0 init:confluentinc/confluent-init-container:2.4.0] replicas:3]]"
